function transformarObjetoEnNodo(item) {
  /* Esta funcion es un constructor de nodos que recibe un objeto del tipo x ejemplo
  {
    tag: "img",
    id: "img1",
  ...
  */
  ...
  if (item.listaClases != undefined)
    item.listaClases.forEach((x) => nuevoNodo.classList.add(x));
  //nuevoNodo.addEventListener("click", item.listaAcciones);
  if (item.listaAcciones != undefined)
    item.listaAcciones.forEach((x) =>
      nuevoNodo.addEventListener(x.evento, x.accion)
    ); //nuevoNodo.addEventListener(x.evento,x.accion))
...
function comprobarUsuario() {
  ...
  let passwordIngresado = document.getElementById(
...
function abrirSesionUsuario(usuario) {
  ...
  mostrarMensaje(
    "Se logueo el usuario: " + usuario + "!!",
    "success",
    "Aceptar"
  );

  let { userName: nombreUsuario, fotoPerfil } =
    baseDatosApp.getUserInfo(usuarioLogueado);
...
function comprobarRegistroUsuario() {
  const usernameIngresado = document.getElementById(
    "register-form-user-input"
  ).value;
  const passwordIngresado = document.getElementById(
    "register-form-password-input"
  ).value;
  const passwordRepeatIngresado = document.getElementById(
    "register-form-re-password-input"
  ).value;
  const emailIngresado = document.getElementById(
    "register-form-email-input"
  ).value;
  const fotoPerfilIngresada = document.getElementById(
    "register-form-perfil-img"
  ).src;
  ...
  if (condicion1 && condicion2 && condicion3 && condicion4) {
    ...
    if (
      baseDatosApp.agregarNuevoUsuario(
        usernameIngresado,
        passwordIngresado,
        emailIngresado,
        fotoPerfilIngresada
      ...
      mostrarMensaje(
        "El usuario " +
          usernameIngresado +
          " se registro exitosamente, ya podes iniciar sesion !!",
        "success",
        "Aceptar"
      );
      ...
      baseDatosApp.agregarNuevoPost(
        baseDatosApp.getUserInfo(usernameIngresado).userID,
        getFechaActual().fechaString,
        getFechaActual().horaString,
        "Bienvenido a Motogram " + usernameIngresado + "subi tu primer foto !",
        PICPOSTBIENVENIDA
      );
    ...
    if (!condicion1)
      mostrarMensaje(
        "El usuario " + usernameIngresado + " ya existe!!",
        "warning",
        "Aceptar"
      );
    if (!condicion2)
      mostrarMensaje(
        "La contraseña ingresada no es seguro ! Utilice al menos 6 letras y solamente numeros y letras !!",
        "warning",
        "Aceptar"
      );
    if (!condicion3)
    ...
    if (!condicion4)
      mostrarMensaje(
        "El email ingresado no es un email valido !!",
        "warning",
        "aceptar"
      );
  }
  ...
  fetch(unplashURLCategoria("alpinestars", 10))
    ...
    .then((data) => {
      ...
      resultados.forEach((x) => {
        ...
        nuevoObjeto = {
          tag: "img",
          id: "perfilPicSelector-pic",
          listaClases: ["selector-picperfil-container-pics"],
          listaAcciones: [
            {
              evento: "click",
              accion: () => {
                document.getElementById("register-form-perfil-img").src =
                  x.urls.thumb;
              },
            },
          ],
      ...
      //Construyo el wrapper donde mostrare las fotos de perfil a elegir y la engancho al main Container
      containerGaleriaFotos = new wrapperElements(
        "id-wrapper-perfilpic-selector",
        "selector-picperfil-container",
        arrayElementos,
...
function selectBackgroundScreen(sesionAbierta) {
  ...
  let categoriaElegida =
    categoriasImagenes[generarValorAleatorio(0, categoriasImagenes.length - 1)];
  ...
  fetch(unplashURLCategoria(categoriaElegida, "20"))
    ...
    .then((data) => {
      ...
      while (!imagenCorrecta) {
        imagenSeleccionada =
          data.results[
            generarValorAleatorio(0, data.results.length - 1)
          ]; /*.urls.small*/
      ...
      const {
        urls,
        width: imagenSeleccionadaWidth,
        height: imagenSeleccionadaHeight,
      } = imagenSeleccionada; //urls es tamnien objeto
      ...
      mainContainer.style.backgroundSize =
        widthContainer.toString() + "px " + backgroundHeight.toString() + "px";
    });
}
        ...
        if (
          ...
          fetch(unplashURLCategoria(categoria, cantidadNuevosPost))
            ...
            .then((data) => {
              ...
              let idUsuarioActual = baseDatosApp.getUserInfo(
                registro.username
              ).userID;
...
function esEmailValido(email) {
  ...
  if (condiciones.test(email)) return true;
...
function getHashTagsList(cantidadMaxima) {
  ...
  while (j < cantidadMaxima) {
    ...
    hashtagList.push(
      hashtagsMotocicletas[
        generarValorAleatorio(0, hashtagsMotocicletas.length - 1)
      ]
    );
    j++;
  }
...
function getFechaAleatoria() {
  ...
  let nuevaFechaHoraAleatoria = DateTime.now().plus({
    days: cantDiasRestar,
    minutes: cantMinutos,
  });
...
function getMinutosTranscurrido(fechaIngresada, horaIngresada) {
  ...
  let fechaHoraIngresadaFormatoISO =
    numAnio + "-" + numMes + "-" + numDia + "T" + numHora + ":" + numMins;
...
tiempo transcurrido */
  let minutosTranscurridos = getMinutosTranscurrido(
    fechaIngresada,
    horaIngresada
  );
  ...
  if (minutosTranscurridos > 10 && minutosTranscurridos <= 59)
    return "Hace " + minutosTranscurridos + " minutos";

  //Entre 1 y 2 horas
  if (minutosTranscurridos > 59 && minutosTranscurridos <= 119)
    return "Hace " + (minutosTranscurridos / 60).toFixed(0) + " hora.";

  //Entre 2 y 23 horas
  if (minutosTranscurridos >= 120 && minutosTranscurridos <= 1439)
    return "Hace " + (minutosTranscurridos / 60).toFixed(0) + " horas";

  ////Entre 1 dia y una semana
  if (minutosTranscurridos >= 1440 && minutosTranscurridos <= 10080)
    return "Hace " + (minutosTranscurridos / 60 / 24).toFixed(0) + " dias";

  ////Entre una semana 4 semanas
  if (minutosTranscurridos >= 10080 && minutosTranscurridos <= 40319)
    return (
      "Hace " + (minutosTranscurridos / 60 / 24 / 7).toFixed(0) + " Semanas"
    );
  // Entre 4 semanas y 12 meses y pedimos el nombre del mes
  if (minutosTranscurridos >= 40320 && minutosTranscurridos <= 525600)
    return (
      +fechaIngresada.slice(0, 2) +
      " de " +
      getNombreMes(fechaIngresada.slice(3, 5))
    );

  ////Un Año o mas
  if (minutosTranscurridos >= 525600)
    return (
      +fechaIngresada.slice(0, 2) +
      " de " +
      getNombreMes(fechaIngresada.slice(3, 5)) +
      " de " +
      fechaIngresada.slice(6, 10)
    );
}
